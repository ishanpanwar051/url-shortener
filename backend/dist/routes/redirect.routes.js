...
router.use('/:shortCode', (req, res, next) => {
    ...
    if (url_service_1.FRONTEND_ROUTES.has(shortCode.toLowerCase())) {
        next('router');
        return;
    }
    next();
...
router.post('/:shortCode/verify', async (req, res) => {
    try {
        ...
        const result = await url_service_1.urlService.getLongUrl(shortCode, ipAddress, userAgent, referer, password);
        if (!result) {
        ...
        logger_1.default.error({ err, shortCode: req.params.shortCode }, 'Password verify failed');
...
router.get('/:shortCode', async (req, res) => {
    try {
        ...
        const userAgent = req.headers['user-agent'] || '';
        const referer = (req.headers['referer'] || req.headers['referrer'] || '');
        const result = await url_service_1.urlService.getLongUrl(shortCode, ipAddress, userAgent, referer);
        if (!result) {
        ...
        logger_1.default.error({ err, shortCode: req.params.shortCode }, 'Redirect failed');
...
function passwordFormHtml(shortCode) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
