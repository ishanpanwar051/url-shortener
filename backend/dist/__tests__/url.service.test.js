...
const mockPrisma = {
    ...
    $transaction: jest.fn().mockImplementation(async (arg) => {
        ...
        if (Array.isArray(arg)) {
            return Promise.all(arg.map((x) => x));
        }
        return undefined;
    }),
    $queryRaw: jest.fn(),
...
const mockRedis = {
    ...
    set: jest.fn().mockImplementation(async (key, value) => { mockRedisStore.set(key, value); return 'OK'; }),
    del: jest.fn().mockImplementation(async (key) => { mockRedisStore.delete(key); return 1; }),
...
describe('UrlService', () => {
    beforeEach(() => {
        ...
        mockRedis.get.mockImplementation(async (key) => mockRedisStore.get(key) ?? null);
        mockRedis.set.mockImplementation(async (key, value) => { mockRedisStore.set(key, value); return 'OK'; });
        mockRedis.del.mockImplementation(async (key) => { mockRedisStore.delete(key); return 1; });
    ...
    describe('createShortUrl', () => {
        it('should create a short URL without custom alias', async () => {
            ...
            const result = await url_service_1.urlService.createShortUrl('https://example.com');
        ...
        it('should create a short URL with custom alias', async () => {
            ...
            const result = await url_service_1.urlService.createShortUrl('https://example.com', undefined, 'myalias');
            expect(result.shortCode).toBe('myalias');
            expect(mockPrisma.uRL.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ customAlias: 'myalias' }) }));
        });
        it('should throw if custom alias already taken', async () => {
            ...
            await expect(url_service_1.urlService.createShortUrl('https://example.com', undefined, 'myalias')).rejects.toThrow('Custom alias already taken');
        });
    });
    describe('getLongUrl', () => {
        it('should return cached URL from LRU', async () => {
            ...
            const result = await url_service_1.urlService.getLongUrl('abc123');
        ...
        it('should return null for non-existent URLs', async () => {
            ...
            const result = await url_service_1.urlService.getLongUrl('nonexistent');
            expect(result).toBeNull();
        });
    });
    describe('getUserUrls', () => {
        it('should return paginated user URLs', async () => {
            ...
            const result = await url_service_1.urlService.getUserUrls(1, 1, 20);
    ...
    describe('deleteUrl', () => {
        it('should delete a URL owned by the user', async () => {
            ...
            await url_service_1.urlService.deleteUrl(1, 1);
        ...
        it('should throw if URL not owned by user', async () => {
            ...
            await expect(url_service_1.urlService.deleteUrl(1, 1)).rejects.toThrow('URL not found or unauthorized');
        });
    });
    describe('updateUrl', () => {
        it('should update a URL owned by the user', async () => {
            ...
            const result = await url_service_1.urlService.updateUrl(1, 1, { longUrl: 'https://updated.com' });
            expect(result.longUrl).toBe('https://updated.com');
            expect(mockRedis.del).toHaveBeenCalledWith('url:abc123');
        });
        it('should throw if URL not owned by user', async () => {
            mockPrisma.uRL.findFirst.mockResolvedValue(null);
            await expect(url_service_1.urlService.updateUrl(1, 1, { longUrl: 'https://x.com' })).rejects.toThrow('URL not found or unauthorized');
        });
    });
    describe('getUrlAnalytics', () => {
        it('should return analytics for a URL owned by the user', async () => {
            ...
            const result = await url_service_1.urlService.getUrlAnalytics('abc123', 1);
            expect(result).not.toBeNull();
            expect(result.totalClicks).toBe(10);
        });
        it('should return null for unowned URLs', async () => {
            mockPrisma.uRL.findFirst.mockResolvedValue(null);
            const result = await url_service_1.urlService.getUrlAnalytics('abc123', 999);
            expect(result).toBeNull();
        });
    });
    describe('parseDevice', () => {
        it('should detect iOS', () => {
            const device = url_service_1.urlService.parseDevice('Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X)');
            expect(device).toBe('iOS');
        });
        it('should detect Android', () => {
            const device = url_service_1.urlService.parseDevice('Mozilla/5.0 (Linux; Android 14) AppleWebKit/537.36');
            expect(device).toBe('Android');
        });
        it('should detect Windows', () => {
            const device = url_service_1.urlService.parseDevice('Mozilla/5.0 (Windows NT 10.0; Win64; x64)');
            expect(device).toBe('Windows');
        });
        it('should detect Mac', () => {
            const device = url_service_1.urlService.parseDevice('Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)');
            expect(device).toBe('Mac');
        });
        it('should detect Bot', () => {
            const device = url_service_1.urlService.parseDevice('Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)');
            expect(device).toBe('Bot');
        });
        it('should return null for undefined user agent', () => {
            const device = url_service_1.urlService.parseDevice(undefined);
